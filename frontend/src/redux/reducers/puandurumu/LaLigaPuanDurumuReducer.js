...
export const fetchLaLigaPuanDurumu = () => {
  return async (dispatch) => {
    try {
      const data = await puanDurumuService.getLaligaPuanDurumu()

      dispatch({
        type: 'FETCH_LALIGA_PUAN_DURUMU_SUCCESS',
        data
      })
    } catch (error) {
      dispatch({
...
const SerieAPuanDurumuReducer = (state = INITIAL_STATE, action) => {
  switch (action.type) {
    case 'FETCH_LALIGA_PUAN_DURUMU_SUCCESS':
      return {
        status: 'success',
        data: action.data
      }
